...
export function createView<Name extends string, Columns extends QueryScope.Columns>(
  name: Name,
  ...
  const $tableData: TableData = { name, columns: Projection.columns(builder.query), constraints: [] }
  return Object.assign(Source.create({ kind: "table", name, tableData: $tableData }), {
    $kind: "view" as const, $meta, $tableData,
